...
class Net(nn.Module):
    def __init__(self, cfg, device, is_train=True, export_onnx_flag=False):
        super(Net, self).__init__()

        self.device = device
        self.is_train = is_train
        self.export_onnx_flag = export_onnx_flag
        ...
        if cfg.network == 'resnet50':
            ...
            modelpretrain = models.resnet18(pretrained)
            dim_modelt = 960
            self.L2pooling_l1 = L2pooling(channels=64)
            self.L2pooling_l2 = L2pooling(channels=128)
            self.L2pooling_l3 = L2pooling(channels=256)
    ...
    def forward(self, x):
        ...
        predictionQA = (self.fc(torch.flatten(torch.cat((fout_t_o, flayer4_o), dim=1), start_dim=1)))
        ...
        elif self.export_onnx_flag:
            return predictionQA
        else:
            return predictionQA, 0
...
class TReS(object):
    ...
    def test(self, data, epochnum, svPath, seed, pretrained=0):
        if pretrained:
            ...
            _path = f'{svPath}/bestmodel_{self.config.vesion}_{seed}.zip'
