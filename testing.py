...
def main(config, device):
    ...
    test_index = [i for i in range(1, 2)]
    train_index = [i for i in range(1,2)]
    ...
    srcc_computed, plcc_computed = solver.test(test_data, version_test_save, pretrained_path, config.seed, pretrained=1)
...
if __name__ == '__main__':
    ...
    dataset_root = '/home/sunnypc/dangxs/datasets/IQA'

    # config.datapath = f'{dataset_root}/LIVE1/data/databaserelease2'
    # config.dataset = 'live'

    config.datapath = f'{dataset_root}/tid2013'
    config.dataset = 'tid2013'

    # CSIQ数据集缺少 csiq_label.txt
    # config.datapath = f'{dataset_root}/CSIQ'
    # config.dataset = 'csiq'

    config.gpunum = '0,'
    config.svpath = './Save_Models_TReS/'
